...
from podracer.digest import due_periods, generate_and_save, init_digest_watermark
...
class Worker:
    ...
    def run_once(self) -> None:
        """Single iteration: sync feeds, enqueue, drain queue, finalize digests."""
        self._sync_feeds()
        self._enqueue_new()
        self._drain_queue()
    ...
    def _schedule_digests(self) -> None:
        """Finalize any due-but-missing (or stale) digest periods. Two cheap
        indexed counts + a tz comparison per recent period; short-circuits well
        before digest_hour. A period left un-generated by an error stays 'due'
        and is retried next tick (self-healing), like a still-queued job."""
        if not self.cfg.digest_enabled:
            return
        init_digest_watermark(self.conn, self.cfg)  # set today's date once
        for period in due_periods(self.conn, self.cfg):
            try:
                generate_and_save(self.conn, self.cfg, period)
                logger.info("digest_generated", kind=period.kind, start=period.start_str)
            except Exception:
                logger.exception("digest_failed", kind=period.kind, start=period.start_str)
                sentry_sdk.capture_exception()
